...
public final class MessageSystemImpl implements MessageSystem {
  ...
  public MessageSystemImpl(
      @Value("${spring.messagesystem.port}") final Integer port
  ) {
    this.port = port;
  ...
  private void handleMessage(MsClient msClient, Message msg) {
    try {
      // TODO: отправляем респонс на изначальное сообщение (только вместо MsClient должен быть Socket clientSocket)
      msClient.handle(msg);
    } catch (Exception ex) {
      logger.error(ex.getMessage(), ex);
